...
export function JobSeekerDashboardEnhanced({ user, jobs, applications, onApplicationUpdate, onLogout }: JobSeekerDashboardEnhancedProps) {
  ...
  const videoResumeInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleVideoResumeUpload = (file: File | null) => {
    if (file) {
      // Validate file type
      const allowedTypes = ['video/mp4', 'video/webm', 'video/ogg', 'video/avi', 'video/mov', 'video/quicktime'];
      if (!allowedTypes.includes(file.type)) {
        toast.error('Please upload a video file (MP4, WebM, OGG, AVI, MOV)');
        return;
      }
      ...
      setProfile({ ...profile, videoResume: file });
      toast.success(`Video resume "${file.name}" uploaded successfully!`);
    }
  };
  ...
  const triggerVideoResumeUpload = () => {
    videoResumeInputRef.current?.click();
  };
  ...
  const getRecommendedJobs = React.useMemo(() => {
    ...
    const jobsWithScores = jobs.map(job => {
      ...
      // Traditional matching (base score)
      ...
      if (matchesSkills) score += 30;
      if (matchesJobType) score += 20;
      if (matchesLocation) score += 15;
      
      // AI-powered score from search history and user behavior
      const aiScore = searchHistoryService.generateRecommendationScore(job);
      score += aiScore * 0.35; // 35% weight for AI recommendations
  ...
  const onViewJob = (jobId: string) => {
    // Track job view interaction for AI recommendations
    searchHistoryService.addUserInteraction({
      type: 'job_view',
      jobId: jobId
    });
  ...
  const onApplyForJob = (jobId: string) => {
    // Track job application interaction for AI recommendations
    searchHistoryService.addUserInteraction({
      type: 'job_apply',
      jobId: jobId
    });
  ...
  const onSaveJob = (jobId: string) => {
    // Track job save interaction for AI recommendations
    searchHistoryService.addUserInteraction({
      type: 'job_save',
      jobId: jobId
    });
                    ...
                    <TabsContent value="professional" className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
                          ...
                          <p className="text-gray-600 mb-2">
                            {profile.videoResume ? `Selected: ${profile.videoResume.name}` : 'Upload your video resume'}
                          </p>
                          <p className="text-sm text-gray-500 mb-4">MP4, WebM, OGG, AVI, MOV up to 50MB</p>
                          <p className="text-xs text-gray-400 mb-4">Record a 1-2 minute self-introduction video</p>
                          <Button 
                            type="button"
                            onClick={triggerVideoResumeUpload}
                            variant="outline"
                          >
                          ...
                          <input
                            ...
                            onChange={(e) => handleVideoResumeUpload(e.target.files?.[0] || null)}
                            className="hidden"
                          />
                        </div>
                      </div>
                ...
                {getRecommendedJobs.map((job) => (
                  <div key={job.id} className="border rounded-lg p-4 hover:shadow-sm transition-shadow">
                    <div className="flex items-start space-x-3 mb-3">
                      <ImageWithFallback
                        src={job.logo || 'https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=40&h=40&fit=crop&crop=face'}
                  ...
                  <div className="text-center p-6 border-2 border-dashed border-gray-300 rounded-lg hover:border-primary/50 transition-colors">
                    {profile.videoResume ? (
                      <div className="space-y-4">
                        ...
                        <p className="text-gray-600 mb-2">
                          Current: {profile.videoResume.name}
                        </p>
                        <div className="bg-gray-100 rounded-lg p-4 mb-4">
                          <div className="flex items-center justify-center space-x-2 text-gray-600">
                            <Play className="w-4 h-4" />
                            <span className="text-sm">Video preview will be available after upload</span>
                          </div>
                        </div>
                        <div className="flex flex-col sm:flex-row gap-2">
                          <Button 
                          ...
                          <Button 
                            variant="outline" 
                            size="sm"
                            onClick={() => toast.success('Video preview opened')}
                            className="w-full sm:w-auto"
                          >
                      ...
                      <div>
                        ...
                        <p className="text-gray-600 mb-2">No video resume uploaded</p>
                        <p className="text-sm text-gray-500 mb-4">Record a 1-2 minute self-introduction</p>
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={triggerVideoResumeUpload}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center p-6 border-2 border-dashed border-gray-300 rounded-lg hover:border-primary/50 transition-colors">
                    ...
                    <p className="text-gray-600 mb-2">
                      {profile.resumeUrl ? `Current: ${profile.resumeFileName || 'Resume available'}` : 'No resume uploaded'}
                    </p>
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Button 
                        variant="outline" 
