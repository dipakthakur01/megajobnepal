...
export function ReportManagement({ jobs, users, applications, companies }: ReportManagementProps) {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      setJobStats(jobRes?.data || jobRes); // backend wraps data under .data
    } catch (err) {
  ...
  const analytics = useMemo(() => {
    ...
    const latest = jobs.length; // treat latest as total newly posted
    ...
    // Approximate new users this month by created_at
