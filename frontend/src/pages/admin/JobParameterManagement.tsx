...
import { getJobParameters, setJobParameters, JobParameters } from '../../services/jobParametersService';
import { confirmDelete } from '../../utils/confirmDelete';

export function JobParameterManagement() {
  const [activeTab, setActiveTab] = useState<keyof JobParameters>('skills');
  const [searchTerm, setSearchTerm] = useState('');
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<any>(null);
  const [parameters, setParameters] = useState<JobParameters>(getJobParameters());
  ...
  const currentData = parameters[activeTab];
    ...
    setParameters(prev => {
      const updated: JobParameters = {
        ...
        [activeTab]: prev[activeTab].map(item =>
          (item as any).id === editingItem.id ? editingItem : item
        ) as any
      };
      setJobParameters(updated);
      return updated;
  ...
  const handleDelete = (id: string) => {
    ...
    setParameters(prev => {
      const updated: JobParameters = {
        ...prev,
        [activeTab]: prev[activeTab].filter(item => (item as any).id !== id) as any
      };
      setJobParameters(updated);
      return updated;
    });
  ...
  const toggleStatus = (id: string) => {
    setParameters(prev => {
      const updated: JobParameters = {
        ...
        [activeTab]: prev[activeTab].map(item =>
          (item as any).id === id
            ? { ...item, status: (item as any).status === 'active' ? 'inactive' : 'active' }
            : item
        ) as any
      };
      setJobParameters(updated);
