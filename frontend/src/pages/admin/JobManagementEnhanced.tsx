...
export function JobManagementEnhanced({ jobs, companies, onJobUpdate }: JobManagementEnhancedProps) {
  ...
  // Functions for JobCard integration
  ...
  const getTierColors = (tier: string) => {
    switch (tier) {
      ...
      default:
        return {
          tierColor: "bg-gradient-to-r from-purple-500 to-violet-500 text-white",
        ...
        <TabsContent value={currentTab} className="mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {filteredJobs.map((job) => {
              const { tierColor, tierBgColor } = getTierColors(job.tier);
              return (
